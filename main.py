...
import httpx
from fastapi import HTTPException, BackgroundTasks, File, UploadFile, Query, Form
...
from config import pipeline_queue, TEMP_DIR, created_instances
...
def create_audio_service(num_pipelines):
    """
    创建并启动音频服务。
    """
...
def create_pipelines_thread(num_pipelines):
    """
    创建并启动多个pipeline实例的线程。
    :param num_pipelines: 要创建的pipeline实例数量
    :return: 启动的线程对象
    """
    thread = threading.Thread(target=create_audio_service, args=(num_pipelines,))
    thread.start()
    return thread


@app.post("/api/v1/upload_chunk", response_model=ChunkUploadResponse, summary="上传文件分片")
...
@app.post("/api/v1/merge_chunks", response_model=TranscribeResponse, summary="合并分片并开始处理")
...
@app.get("/api/v1/tasks/{task_id}/status", response_model=TaskStatusResponse, summary="获取任务状态")
...
@app.get("/api/v1/tasks/{task_id}/segments",
...
@app.get("/api/v1/download/single/{task_id}/{segment_index}", responses={
...
@app.get("/api/v1/download/bulk/{task_id}", responses={
...
@app.get("/api/v1/download/all/{task_id}", responses={
...
@app.delete("/api/v1/segments/{task_id}", summary="批量删除指定分段")
...
@app.get("/api/v1/speakers/{task_id}", summary="获取发音人列表")
